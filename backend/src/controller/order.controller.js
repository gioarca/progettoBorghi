...
// Endpoint per creare un ordine: prende i dati dai modelli creati Borgo e user
const addOrder = async (req, res) => {
  try {
    const { Borgos, user } = req.body;
    const order = new Order({ Borgos, user });
    await order.save();
...
const getOrders = async (req, res) => {
  try {
    ...
    const { BorgoId, createdAt } = req.params;
    const orders = await Order.find({
      Borgos: BorgoId,
      createdAt,
    })
      .populate("Borgos users") // per il populate bastano i due schemi dichiarati nel model
      .limit(limit)
